...
- **booked_day derivation** (per deal, in `deal2`/getCall1Deals): the **earliest booking signal** — first `BOOKING_COMPLETE` (the true booking action; it carries no email, so it's bridged email→amplitude_id→booking_complete) or the `Call 1 Scheduled` stage event, whichever is earlier — with the `booking_overrides` tab winning over both. BOOKING_COMPLETE is earlier/truer than the CRM event in ~99% of disagreements (Call 1 Scheduled is often a late re-book). This auto-fills all but ~13 accepted/held deals; those (no signal at all) need a manual override.
