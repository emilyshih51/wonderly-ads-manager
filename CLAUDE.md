...
- **HELD** = deal moved _past_ "Call 1 Scheduled" to any real disposition (positive stage-change event, OR current CRM stage in Accepted/Reviewing Contract/Quote Sent/On-site/Signed/Won/Churned/Disqualified/DQ). NOT held if still in "Call 1 Scheduled" or "Call Missed Several Times" (no-show). Held/accepted deals reach post-call stages that DO fire stage-change events, so the event-based `deal2`/`ds` set captures them.
- **NO_SHOW** = the no-show stage ("Call Missed Several Times"), **enumerated straight from `CRM_DEALS`** (CTEs `nsc`/`nscs` in `getDailyMarketing`), NOT the stage-change events — that transition fires **no event**, so the event-based set misses ~75% (~118 of ~469). Keyed on stage **`TYPE = 'meeting_no_show'` scoped to the acquisition pipeline** (`ACQUISITION_PIPELINE_ID = pip_019c0568…`), not the display NAME — `TYPE` is a stable enum that survives renames, and the pipeline scope drops contractor job deals that reuse the same stage name across 1000+ pipelines (545 by name → 527 scoped; 18 were contractor deals). Booked/accepted/held stay name-keyed (their types are `custom` or shared — e.g. Accepted's `meeting_finished` is also "Call 2 Scheduled (yet to pay)"). Cohort-keyed by booking day (BOOKING*COMPLETE email bridge, else CRM `CREATED_AT`, else override), split FB/organic via email→utm. ~460 since May, matches the internal CSM Ops tool. \*\*Do NOT source held/no-show from the prod `DERIVED__CUSTOMER_FUNNEL` meeting model — that's the \_customers'* funnel (contractors↔homeowners), a disjoint population (see `docs/call1-deals-prod-rebuild-scope.md`).\*\* `call1_deals`/`ad_performance` still enumerate no-show from the event set (per-deal audit), so their no-show undercounts — a known gap, not yet migrated.
- **ACCEPTED** = ever fired the `Accepted` event (milestone; stays true after churn/drop) OR currently in a post-acceptance stage (Accepted/Reviewing Contract/Quote Sent/On-site/Signed/Won). Validated ~148 since May (matches CSM Ops ~142). Enumerated from the event-based set (`ds`) — kept unchanged.
