...
- **HELD/ACCEPTED are COHORT-keyed by booking day** (of the deals booked that day, how many eventually held/accepted) in **Daily Metrics / Daily Funnel / historical_cac**. Recent days read low until they mature. The rest of the funnel (page views → booked) is flow-keyed by event date. **The Overview is different**: its headline "Cost per accepted contractor" (7d) and the week-over-week HELD/ACCEPTED rows are **flow-keyed by event date** (counted from `call1_deals` `acceptedDate` / `heldDate`), so a recent window isn't zeroed out by cohort immaturity and the headline reconciles with the w/w table. (The Overview "Cost per accepted contractor (30d)" row is still the cohort version.)
