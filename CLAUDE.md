...
- **HELD / NO_SHOW (wonderly_daily, Daily Metrics, Daily Funnel)** = the **prod meeting-outcome model** (`WONDERLY_DATA.DERIVED__CUSTOMER_FUNNEL.INT__CUSTOMER_FUNNEL_V2_MEETING_OUTCOMES`): `meeting_finished` = held, `meeting_no_show` (and never finished — excludes reschedules that later held) = no-show. **FLOW-keyed by the call/outcome date** (`EVENT_DATE`), not the booking day, and **ALL-only** (no FB/Organic split — prod leads don't bridge to the marketing utm signal; email overlap ~0). This replaces the old CRM-stage held/no-show: booking-day cohorting in prod covers only ~8% of outcomes, and the old "current stage = Call Missed Several Times" undercounts no-shows ~2× (they sit stranded in "Call 1 Scheduled"). Because it's flow-keyed by call date there's no maturation lag. The new CTEs (`mo`/`hc`/`hcs`) live in `getDailyMarketing`; `held_fb/held_organic/no_show_fb/no_show_organic` are emitted as 0. **call1_deals still uses the CRM stage milestone for held/no-show** (a known inconsistency with the sheet — not yet migrated).
- **ACCEPTED** = ever fired the `Accepted` event (milestone; stays true after churn/drop) OR currently in a post-acceptance stage (Accepted/Reviewing Contract/Quote Sent/On-site/Signed/Won). **Kept on the CRM-stage source, COHORT-keyed by booking day** (unchanged) — only held/no-show moved to the attendance model.
- **ACCEPTED is COHORT-keyed by booking day** (of the deals booked that day, how many eventually accepted) everywhere — Daily Metrics, Daily Funnel, historical_cac, AND the Overview. Recent windows read low/$0 until the cohort matures; the 30d and historical_cac are the matured figures. HELD/NO_SHOW are flow-keyed (above), so `held_rate = held ÷ booked` in the Daily Funnel mixes a call-date numerator with a booking-day denominator (approximate). The rest of the funnel (page views → booked) is flow-keyed by event date.
