...
- **HELD / NO_SHOW (wonderly_daily, Daily Metrics, Daily Funnel)** = the **prod meeting-outcome model** (`WONDERLY_DATA.DERIVED__CUSTOMER_FUNNEL.INT__CUSTOMER_FUNNEL_V2_MEETING_OUTCOMES`): `meeting_finished` = held, `meeting_no_show` (and never finished — excludes reschedules that later held) = no-show. **COHORT-keyed by the deal's BOOKING day** (`INT__CUSTOMER_CRM_DEAL_DETAILS.DEAL_CREATED_TIME` = day the Call 1 was booked, 100% coverage) — of the leads booked that day, how many held / no-showed. **ALL-only** (no FB/Organic split — prod leads don't bridge to the marketing utm signal; email overlap ~0). Replaces the old CRM-stage held/no-show, which undercounts no-shows ~2× (stranded in "Call 1 Scheduled"). Like ACCEPTED, recent booking days read low until the cohort's calls happen (join `mo` outcomes → `dd` deal details for the booking day; CTEs `mo`/`hc`/`hcs` in `getDailyMarketing`). `held_fb/held_organic/no_show_fb/no_show_organic` are emitted as 0. **call1_deals still uses the CRM stage milestone for held/no-show** (a known inconsistency — not migrated; see `docs/call1-deals-prod-rebuild-scope.md`).
- **ACCEPTED** = ever fired the `Accepted` event (milestone; stays true after churn/drop) OR currently in a post-acceptance stage (Accepted/Reviewing Contract/Quote Sent/On-site/Signed/Won). **Kept on the dev CRM-stage source, COHORT-keyed by booking day** (unchanged, validated ~148 since May 1). Does NOT reconcile to any prod milestone — a definition call for the metric owner; do not migrate without RevOps sign-off (see the scope doc).
- **Everything is COHORT-keyed by booking day** — HELD/NO_SHOW (attendance model) and ACCEPTED (CRM stage) alike — so of the deals booked that day, how many held / no-showed / accepted. `held_rate = held ÷ booked` and `no_show_rate` are clean same-cohort ratios. Recent windows read low until the cohort matures; historical_cac is the matured figure. The rest of the funnel (page views → booked) is flow-keyed by event date.
