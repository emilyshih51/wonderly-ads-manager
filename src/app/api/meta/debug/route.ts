...
export async function GET() {
  ...
  // 1. Fetch campaigns
  try {
    const campaigns = await metaApi(`/act_${session.ad_account_id}/campaigns`, session.meta_access_token, {
      ...
      (campaigns.data || []).map(async (c: { id: string; name: string }) => {
        try {
          const insights = await metaApi(`/${c.id}/insights`, session.meta_access_token, {
            params: {
              fields: 'spend,impressions,clicks,ctr,cpc,cpm,reach,actions,cost_per_action_type,date_start,date_stop',
              date_preset: 'today',
            },
          });
          return { campaign_id: c.id, campaign_name: c.name, insights_rows: insights.data, raw_response: insights };
        } catch (err) {
          return { campaign_id: c.id, campaign_name: c.name, error: err instanceof Error ? err.message : String(err) };
        }
      })
    );
  ...
  // 3. Fetch ad sets
  try {
    const adSets = await metaApi(`/act_${session.ad_account_id}/adsets`, session.meta_access_token, {
      params: {
  ...
  try {
    const adSetsFull = await metaApi(`/act_${session.ad_account_id}/adsets`, session.meta_access_token, {
      params: {
