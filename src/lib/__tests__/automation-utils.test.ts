...
describe('getResultCount()', () => {
  ...
  it('returns 0 when mapped type not found in actions (no fallthrough to unrelated conversions)', () => {
    // Ad set is mapped to fb_pixel_lead, but actions only contain start_trial.
    // The exact and fuzzy match both fail — return 0 because the ad set's optimization
    // goal is known and the ad has 0 of that specific conversion. The generic fallback
    // would incorrectly count the unrelated start_trial action.
    const row = {
      campaign_id: 'c1',
      actions: [
