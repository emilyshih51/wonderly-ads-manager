...
describe('buildDailyMetricsFormatRequests', () => {
  ...
  it('merges one group header per metric across its seven columns', () => {
    const merges = reqs.filter((r) => 'mergeCells' in r) as any[];

    expect(merges).toHaveLength(METRICS.length);
    // First metric (Spend) merges cols 1..8 (ALL, w/w, FB, Google, Yahoo, Bing, N/A).
    ...
    // Third metric (Page views) starts at col 15 (1 + 2*7).
    expect(merges[2].mergeCells.range.startColumnIndex).toBe(15);
  });
  ...
  it('adds a heat-map on every w/w column', () => {
    ...
    // w/w columns are 2, 9, 16 (1 + g*7 + 1).
