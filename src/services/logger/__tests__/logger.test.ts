...
describe('Logger — onError reporter', () => {
  beforeEach(() => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.spyOn(console, 'warn').mockImplementation(() => {});
  });
  ...
  it('calls onError for error-level entries', () => {
    const onError = vi.fn();
    const logger = createLogger('Test', { onError });
  ...
  it('does not call onError for warn-level entries', () => {
    const onError = vi.fn();
    const logger = createLogger('Test', { onError });
  ...
  it('passes a LogEntry with error fields when data is an Error', () => {
    const onError = vi.fn();
    const logger = createLogger('MyScope', { onError });
    const err = new Error('test error');

    logger.error('it failed', err);

    const entry = onError.mock.calls[0][0];

    expect(entry.scope).toBe('MyScope');
    expect(entry.message).toBe('it failed');
  ...
  it('passes a LogEntry with data field when data is a plain object', () => {
    const onError = vi.fn();
    const logger = createLogger('Test', { onError });
    ...
    expect(entry.data).toEqual({ requestId: '123' });
    expect(entry.error).toBeUndefined();
  });

  it('includes timestamp and scope in LogEntry', () => {
    const onError = vi.fn();
    const logger = createLogger('Evaluate', { onError });
    ...
    expect(entry.scope).toBe('Evaluate');
  });
});
